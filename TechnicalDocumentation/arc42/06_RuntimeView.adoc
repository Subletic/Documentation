...
image::ClientCommunication.drawio.svg[Static,100%,align="center"]

Möchte sich ein _Client_ mit dem Backend verbinden, so kann er dies über den ClientExchangeController_ tun. _Subletic_ startet nach dem Programmstart einen WebSocket-Server und wartet auf den Verbindungsaufbau mit einem WebSocket-_Client_. Nachdem dieser erfolgt ist, wird vom _Client_ erwartet, dass dieser eine initiale Message schickt in dem das Untertitel-Format spezifiziert wird. Passiert dies nicht, so wird die Verbindung geschlossen. Und wieder auf eine neue Verbindung gewartet. Wird ein gültiges Format übergeben, kann der richtige _ISubtitleConverter_ gesetzt werden, und _Subletic_ ist bereit für den Entgegennahme von Audio-Paketen. Für jedes ankommende Paket wird die Untertitelerstellung angestoßen, und getrennt von diesem Prozess, bei Fertigstellung des Untertitels an den _Client_ zurück geschickt. Gleichzeitig wird auf das nächste Paket gewartet. Wird die Verbindung vom _Client_ getrennt, bzw. kommen keine Pakete mehr an, so wird nach einem Timeout die Verbindung geschlossen und auf eine neue Verbindung gewartet.
